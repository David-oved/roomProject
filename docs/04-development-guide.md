...
### בדיקה ידנית מהירה — `npm run demo`

כשרוצים **לראות** שינוי עובד ולא רק שהבדיקות ירוקות, יש קיצור בפקודה
אחת שלא דורש להכין `.env.local` ולא דורש להזין נתונים ביד:

```bash
npm run demo
```

מה הוא עושה: מרים אמולטורים (Auth + Database), זורע חדר בדיקות ייעודי,
ומריץ את שרת הפיתוח מול `.env.demo` — קובץ עם ערכי Firebase מזויפים
ו-`VITE_USE_EMULATORS=true`. הכול נסגר יחד ב-Ctrl+C, ושום דבר לא נוגע
בייצור.

| | |
|---|---|
| כתובת | http://127.0.0.1:5173 |
| כניסה (מנהל) | `demo.david@example.com` / `roommate123` |
| חדר | `DEMO01` — "חדר בדיקות — מטלות" |

חדר הבדיקות (`scripts/seed-task-demo.mjs`) בנוי סביב תרחישים שקשה
להכין ביד, והסקריפט מדפיס לטרמינל מה אמור לקרות בכל אחד — כולל
המספרים המדויקים לפני ואחרי. הוא מוסיף בלבד (`update`), ולכן אפשר
להריץ אותו גם על גבי `npm run emu:seed` בלי לדרוס אותו.

‼️ **הכללים נאכפים גם באמולטור.** זה הופך את `npm run demo` לדרך
היחידה לבדוק שינוי ב-`database.rules.json` לפני שהוא עולה — פער בין
קוד שכבר בייצור לכללים שעדיין לא נפרסו הוא בדיוק התקלה שתועדה
ב-v1.22.0 (ראו ההערה בסוף `.github/workflows/deploy.yml`). כדי לוודא
שכלל באמת אוכף ולא סתם "עבר", שווה להריץ פעם אחת עם הכלל הישן
ולראות את הפעולה נחסמת.
